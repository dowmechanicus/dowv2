...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  if (Object.keys(context.query).length) {
    ...
    const hero: string | undefined =
      (context.query?.hero as string) || undefined;
    ...
    if (Array.from(search_params).length) {
      const query_response = await fetch(
        `${process.env.API_URL}:${process.env.PORT}${process.env.BASE_PATH}/api/search?` +
          search_params
      );
